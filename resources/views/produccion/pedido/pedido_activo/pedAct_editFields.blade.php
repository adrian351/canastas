<div class="row">
  {{--  campo: lider de padido deshabilitado  --}}
  ...
  {{-- asignar bodega de produccion por defecto --}}
    <div class="form-group col-sm btn-sm">
      ...
      <div class="input-group">
        ...
        {{--  {!! Form::select('bodega_donde_se_armara', config('opcionesSelect.select_bodega_donde_se_armara'), $pedido->bod, ['class' => 'form-control select2' . ($errors->has('bodega_donde_se_armara') ? ' is-invalid' : '')]) !!}  --}}
        <select class="form-control select2" aria-label="Default select example" placeholder="selecciona" name="bodega_donde_se_armara">
          @if ($pedido->arm_carg >= 21)
              <option value="Temas" selected>Temas</option>
              <option value="Naucalpan">Naucalpan</option>

              @else
              <option value="Naucalpan" selected>Naucalpan</option>
